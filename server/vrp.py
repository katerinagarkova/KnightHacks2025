...
# You can also define multiple arc cost evaluators that depend on which vehicle is traveling between locations, 
# using the method routing.SetArcCostEvaluatorOfVehicle(). For example, if the vehicles have different speeds, 
# you could define the cost of travel between locations to be the distance divided by the vehicle's speed — in other words, the travel time.
routing.SetArcCostEvaluatorOfAllVehicles(transit_callback_index)

# Add Distance Constraint (max distance possible per drone)
dimension_name = "Distance"
routing.AddDimension(
    transit_callback_index,
    0, # no slack
    3700, # max individual distance of 3700 ft before battery degredation
...
def print_solution(manager, routing, solution, filename=f"solution_for_{data.get('num_vehicles')}.txt"):
    ...
    with open(filename, "w") as f:
        f.write(f"Objective: {solution.ObjectiveValue()} feet\n")
        for vehicle_id in range(data["num_vehicles"]):
            ...
            plan_output += f" {manager.IndexToNode(index)} \n"
            plan_output += f"Route distance: {route_distance} feet\n"
            f.write(plan_output)
